...
class MediaRuntime:
    def __init__(
        ...
        self.pending_notifications: dict[int, dict] = {}
    ...
    async def _process_source_message(
        ...
        await self._register_new_media_notification(
            source_chat_id,
            kind,
        )
    ...
    async def _register_new_media_notification(
        ...
        current = self.notification_tasks.get(source_chat_id)
        if current and not current.done():
            return

        self.notification_tasks[source_chat_id] = asyncio.create_task(
            self._send_grouped_notification(source_chat_id)
        )
    ...
    async def _send_grouped_notification(
        ...
        bucket = self.pending_notifications.get(source_chat_id)
        if not bucket:
            return

        chat_name = str(source_chat_id)
        try:
            entity = await self.client.get_entity(source_chat_id)
            chat_name = (
            ...
            log.exception("Could not resolve source name")

        counts = bucket["counts"]
        lines = []

        if counts["video"] > 0:
            lines.append(f"🎬 Videos: <b>{counts['video']}</b>")
        if counts["photo"] > 0:
            lines.append(f"🖼 Images: <b>{counts['photo']}</b>")
        if counts["audio"] > 0:
            lines.append(f"🎵 Audio: <b>{counts['audio']}</b>")
        if counts["file"] > 0:
            lines.append(f"📁 Files: <b>{counts['file']}</b>")
        ...
        text = (
            ...
            f"Uploaded to Database: <b>{bucket['uploaded']}</b>\n"
        ...
        try:
            ...
            log.exception("New media notification failed")
        finally:
            self.pending_notifications.pop(source_chat_id, None)
            self.notification_tasks.pop(source_chat_id, None)
